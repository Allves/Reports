...
> Atualizado em 29/07/2026 (v3 — Representante e Administradora). Este documento compara os módulos funcionais em
...
- **Cadastros Register**: permanecem como pendências os submódulos de outros cadastros (comissão, QDD e layout extrato), filtros avançados, exportação server-side, Credenciadora activate e Blacklist delete (ambos A INVESTIGAR NA API).
...
| Administradoras | ✅ Concluído no escopo atual | Lista preservada; lookup CNPJ com enriquecimento/reuso de Pessoa; detalhe em oito abas; dados PJ, endereços, contatos e QSA; Complemento, Representantes, Status, Tipos de Recebíveis, Anexos, Observações e Histórico; BFFs, React Query, claims e E2E específicos. Contrato: `representantes-administradoras.md`. | Validar payloads contra ambiente integrado e ampliar filtros/exportação somente em recorte próprio, sem reabrir o CRUD migrado. |
...
| Representantes (cadastro) | ✅ Concluído no escopo atual | Lista preservada; lookup CPF/CNPJ; criação e edição PF/PJ; Pessoa, endereços, contatos e QSA; abas Documentos, Anexos, Observações e Histórico; catálogo normal/QI Tech, claims e E2E específicos. Contrato: `representantes-administradoras.md`. | Validar payloads e operações multipart contra ambiente integrado; evoluções futuras devem seguir o contrato canônico de formulários. |
...
| 2 — Cadastros estruturantes | Sprints 6–8 — Representante e Administradora concluídos em 29/07/2026 | **Representante e Administradora completos no recorte migrado**, incluindo componentes compartilhados de Pessoa, anexos, observações e histórico. Restam nos demais cadastros: comissão, QDD, layout extrato, filtros avançados, exportação server-side e módulos FIDC/Empresas/Regulamento. | "Os dois cadastros estruturantes mais complexos deste recorte estão operacionais e deixaram uma base reutilizável para os próximos formulários." | Validar integrações multipart e payloads runtime no ambiente integrado; evitar regressão ao adotar os componentes comuns em outros módulos. | Priorizar a adoção controlada dos componentes compartilhados e os submódulos remanescentes, sem copiar o protótipo histórico. | Representante e Administradora validados; componentes canônicos documentados; pendências dos demais cadastros replanejadas. |
